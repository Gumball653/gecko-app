...
export default function AnimalQrTrackingApp() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-3 pb-24 text-slate-900 sm:p-4 md:p-8">
                ...
                <CardContent className="p-4 sm:p-5">
                  <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1 md:flex-wrap md:overflow-visible">
                      ...
                      <Button variant="outline" onClick={() => setShowQr(true)} className="rounded-xl"><Icon name="qr" className="mr-2 h-4 w-4" /> QR Code</Button>
      ...
      {showQr && selected && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-slate-900/60 p-0 sm:items-center sm:p-4"><Card className="max-h-[92vh] w-full max-w-md overflow-auto rounded-t-3xl border-slate-200 shadow-2xl sm:rounded-3xl"><CardContent className="space-y-5 p-4 text-center sm:p-6"><div className="flex items-center justify-between gap-4 text-left"><div><h3 className="flex items-center gap-2 text-2xl font-bold"><Icon name="qr" className="h-6 w-6" /> Printable QR code</h3><p className="text-sm text-slate-500">{selected.name} - {selected.id}</p></div><Button variant="ghost" onClick={() => setShowQr(false)} className="rounded-xl"><Icon name="close" className="h-5 w-5" /></Button></div><div className="mx-auto w-fit rounded-3xl border border-slate-200 bg-white p-5 shadow-sm"><div className="grid grid-cols-[repeat(13,12px)] gap-0.5 rounded-xl bg-white p-3 ring-1 ring-slate-900">{buildQrCells(selected.qrCode).map((filled, index) => <span key={index} className={`h-3 w-3 ${filled ? "bg-slate-900" : "bg-white"}`} />)}</div><p className="mt-4 text-xl font-bold tracking-wide">{selected.qrCode}</p><p className="text-xs text-slate-500">Unique code for this profile</p></div><div className="flex justify-end gap-2"><Button variant="outline" onClick={() => setShowQr(false)} className="rounded-xl">Close</Button><Button onClick={printQrCode} className="rounded-xl"><Icon name="save" className="mr-2 h-4 w-4" /> Print QR code</Button></div></CardContent></Card></div>
      )}
