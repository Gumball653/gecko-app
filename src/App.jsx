...
function deleteSelectedAnimal() {
 ...
 async function printQrCode() {
  ...
  {activeTab !== "housingProfile" &&
    ["profile", "housing", "photos", "breeding", "logs", selected.stage === "egg" ? "egg" : null]
      .filter(Boolean)
      .map((tab) => (
        <Button
          key={tab}
  ...
  {activeTab !== "housingProfile" && (
    <>
      <Button
        variant="outline"
        onClick={() => setRealQrProfile(selected)}
        className="rounded-xl"
      >
      ...
      <Button
        ...
        className="rounded-xl"
      >
        <Icon name="branch" className="mr-2 h-4 w-4" /> Pedigree
      </Button>
                  ...
                  <CardContent className="grid gap-4 p-4 sm:p-5 md:grid-cols-2">
                    ...
                    <div className="md:col-span-2 flex flex-col gap-2 sm:flex-row sm:justify-end">
                      ...
                      <Button
                        ...
                        className="rounded-xl"
                      >
                        <Icon name="save" className="mr-2 h-4 w-4" /> Save profile log
                      </Button>
                    </div>
                    </CardContent>
                  </Card>
         ...
         <Field label="Humidity">
          ...
          />
                ...
                onClick={() => {
                  ...
                  if (location) {
                    setSelectedLocationId(location.id);
                    setRealQrProfile(location);
                  }
                }} 
                 
                  className="rounded-xl" disabled={!selected.housing?.locationId}><Icon name="qr" className="mr-2 h-4 w-4" /> View location QR</Button>
                  <Button
                    variant="outline"
                    onClick={markSelectedAnimalSprayed}
                      className="rounded-xl"
                  >
